...
import { db, firebaseListo, authReady } from "../firebase";
import {
...
export function useCollection(colName, localKey, defaultVal = []) {
  ...
  useEffect(() => {
    ...
    let unsub = () => {};
    let cancelado = false;
    // 🔒 Espera a que la autenticación anónima esté lista antes de consultar — así las reglas de
    // seguridad de Firestore (que exigen estar autenticado) no rechazan la primera consulta.
    authReady.then(() => {
      if (cancelado) return;
